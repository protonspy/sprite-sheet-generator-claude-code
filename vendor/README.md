...
`LICENSE` is upstream's, and it stays with the module. `pixel-snapper.wasm.sha256` is the
module's digest, written by the build and checked by the suite: the binary cannot change
without that one line changing with it.
