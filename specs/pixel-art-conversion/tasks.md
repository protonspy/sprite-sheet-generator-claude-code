...
- [x] 3.1 (TDD) Compute one palette across a frame set and map every frame onto it, leaving alpha alone — R3.1, R3.2, R3.3
      **RED observed** — `tests/core/test_pixelart.py` was written first and failed to
      collect (`ModuleNotFoundError: No module named 'ssc.core.pixelart'`), then went green
      on nine tests once the module existed. The test carrying the requirement is
      `test_a_region_that_did_not_move_is_identical_across_frames`: the same static region
      in two frames must come back byte-identical, which is what per-frame quantization
      cannot do.
- [x] 3.2 (Unit) Ordered and Floyd-Steinberg dithering against a fixed palette, and neither by default — R3.4, R3.5
- [x] 3.3 (Unit) Orphan-cluster cleanup, and one-pixel outline emphasis — R3.6, R3.7
- [x] 3.4 (Unit) Build `ssc tool pixelart` over the four — R3.1, R3.4, R3.6, R3.7

## 4 · `board`

- [x] 4.1 (Unit) Generate the checkerboard and the pose board, each reporting the layout it wrote — R4.1, R4.2, R4.3
