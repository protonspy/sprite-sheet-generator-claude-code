...
- [x] 1.1 (Unit) Define a profile and the built-ins — the six at first, `background` with `layered` as `specs/parallax-layers/`' delta, and `box-art` with no applicable checks as `specs/gen-fal/`'s — R1.1, R1.2
...
- [x] 1.4 (Unit) Read `ssc.yaml` as untrusted: bound it in one open, check its shape at every level, refuse anchors — R1.5
